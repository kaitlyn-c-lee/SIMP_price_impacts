...
MakeForecasts <-function(y, candidate.models, xreg = NULL, dates,
  ...
  candidate.models <- candidate.models %>% arrange(ic) %>% mutate(delta_i= abs(min(ic) - ic)) # NEW

  ### STEP 2. Calculate Akaike weights
  candidate.models <- candidate.models %>% filter(!is.infinite(delta_i) & abs(delta_i)<2) # remove really bad models 
